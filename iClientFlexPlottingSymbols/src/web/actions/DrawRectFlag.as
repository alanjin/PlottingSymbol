package web.actions
{
	 ...
	 * 用于绘制直角旗标
	 */
	public class DrawRectFlag extends DrawAction
	{
		/**
		 * 定义直角旗标字段
		 * 用于记录当前正在绘制的直角旗标
		 */
		private var _geoRectFlag:GeoRectFlag;
		/**
		 * 构造函数
		 *  @param map 初始化时需要关联的地图
		 */
		public function DrawRectFlag(map:Map)
		{
			super(map);
		...
		override protected function onMouseClick(event:MouseEvent):void
		{
			...
			startDraw(point2D);
			//初始化直角旗标，多添加一个点
			this._geoRectFlag = new GeoRectFlag([point2D,point2D]);
			//设置风格
			this.tempFeature.style = this.style;
			//设置几何对象
			this.tempFeature.geometry = this._geoRectFlag;
			//添加到临时图层中
		...
		private function onMouseEndClick(event:MouseEvent):void
		{
			...
			//直接跟换第二个点
			this._geoRectFlag.controlPoints[1] = point2D;
			//重新设置一下，触发旗标计算
			this._geoRectFlag.controlPoints = this._geoRectFlag.controlPoints;
			//设置绘制处于结束
