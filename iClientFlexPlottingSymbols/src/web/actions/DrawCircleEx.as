package web.actions
{
	 ...
	 * 此类是针对圆集合对象的编辑
	 */
	public class DrawCircleEx extends DrawAction
	{
		/**
		 * 定义圆字段
		 * 用于记录当前正在绘制的圆对象
		 */
		private var _geoCircle:GeoCircle;
		/**
		 * 构造函数
		 *  @param map 初始化时需要关联的地图
		 */
		public function DrawCircleEx(map:Map)
		{
			super(map);
		...
		{//设置开始绘制
			...
			startDraw(point2D);
			//初始化圆，多添加一个点
			this._geoCircle = new GeoCircle([point2D,point2D]);
			//设置风格
			this.tempFeature.style = this.style;
			//设置几何对象
			this.tempFeature.geometry = this._geoCircle;
			//添加到临时图层中
		...
		private function onMouseEndClick(event:MouseEvent):void
		{
			...
			//直接跟换第二个点
			this._geoCircle.controlPoints[1] = point2D;
			//重新设置一下，触发旗标计算
			this._geoCircle.controlPoints = this._geoCircle.controlPoints;
			//设置绘制处于结束
