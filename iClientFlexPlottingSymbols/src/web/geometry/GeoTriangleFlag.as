package web.geometry
{
	 ...
	 * 使用两个控制点直接创建直角旗标
	 */
	public class GeoTriangleFlag extends GeoPlotting
	{
		/**
		 * 构造函数
		 * @param points 需要传入的控制点（理论上应该是两个），默认为null 
		 */
		public function GeoTriangleFlag(points:Array=null)
		{
			super(points);
		}
		/**
		...
		override protected function calculateParts():void
		{
			...
			if(this._controlPoints.length>1)
			{
				...
				var point2:Point2D = new Point2D(endPoint.x,(startPoint.y+endPoint.y)/2);
				var point3:Point2D = new Point2D(startPoint.x,(startPoint.y+endPoint.y)/2);
				var point4:Point2D = new Point2D(startPoint.x,endPoint.y);
				this.addPart([point1,point2,point3,point4]);
			}
		}
		/**
		...
		{
			var geoTriangleFlag:GeoTriangleFlag=new GeoTriangleFlag();
			...
			geoTriangleFlag.controlPoints = controlPoints;
			return geoTriangleFlag;			
		}
		
		/**
		 * 根据json数据转换为GeoTriangleFlag对象
		 * @param str json数据
		 * @return 返回的GeoTriangleFlag对象
		 */
		public static function fromJSON(str:String):GeoTriangleFlag
		{
			var geoTriangleFlag:GeoTriangleFlag = new GeoTriangleFlag();
			//匹配控制点的数据
			//取第二个代表获取括号内匹配的
			...
			geoTriangleFlag.controlPoints = arr;
			return geoTriangleFlag;
		}
