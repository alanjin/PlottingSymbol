package web.actions
{
	...
	import web.geometry.GeoDiagonalArrow;
	 ...
	 * 用于快速绘制斜箭头
	 */
	public class DrawDiagonalArrow extends DrawAction
	{
		/**
		 * 定义斜箭头字段
		 * 用于记录当前正在绘制的斜箭头
		 */
		private var _geoDiagonalArrow:GeoDiagonalArrow;
		/**
		 * 构造函数
		 * @param map 初始化时需要关联的地图
		 */
		public function DrawDiagonalArrow(map:Map)
		{
			super(map);
		...
		override protected function onMouseClick(event:MouseEvent):void
		{
			...
			startDraw(point2D);
			//初始化斜箭头，多添加一个点
			this._geoDiagonalArrow = new GeoDiagonalArrow([point2D,point2D]);
			//设置风格
			this.tempFeature.style = this.style;
			//设置几何对象
			this.tempFeature.geometry = this._geoDiagonalArrow;
			//添加到临时图层中
		...
		private function onMouseNextClick(event:MouseEvent):void
		{
			...
			//添加新的控制点
			this._geoDiagonalArrow.controlPoints.push(point2D);
			//重新设置控制点数组，触发绘制点计算
			this._geoDiagonalArrow.controlPoints = this._geoDiagonalArrow.controlPoints;
			//刷新feature
		...
		override protected function onMouseMove(event:MouseEvent):void
		{
			...
			//直接跟换最后一个控制点
			this._geoDiagonalArrow.controlPoints[this._geoDiagonalArrow.controlPoints.length-1] = point2D;
			//重新设置控制点数组，触发绘制点计算
			this._geoDiagonalArrow.controlPoints = this._geoDiagonalArrow.controlPoints;
			//刷新feature
			this.tempFeature.refresh();
		...
		override protected function onMouseDoubleClick(event:MouseEvent):void
		{
			...
			//如果控制点少于两个，这返回
			if(this._geoDiagonalArrow.controlPoints.length < 2)
			{
				return;
			}
			//删除最后一个控制点（结束时最后一个是重复的控制点）
			var endPoint:Point2D = this._geoDiagonalArrow.controlPoints.pop();
			//重新设置控制点数组，触发绘制点计算
			this._geoDiagonalArrow.controlPoints = this._geoDiagonalArrow.controlPoints;
			//设置绘制处于结束
